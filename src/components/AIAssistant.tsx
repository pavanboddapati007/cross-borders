...
const AIAssistant = () => {
  ...
  return <div className="min-h-screen bg-gray-950 flex flex-col">
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-4 py-6">
        ...
        <div className="space-y-6">
          {messages.map(message => <div key={message.id} className={`flex space-x-4 ${message.isUser ? 'flex-row-reverse space-x-reverse' : ''}`}>
              ...
              <div className={`flex-1 ${message.isUser ? 'text-right' : ''}`}>
                <div className={`p-4 rounded-xl max-w-3xl ${message.isUser ? 'bg-blue-600 text-white ml-auto' : 'bg-gray-800 text-gray-100 border border-gray-700'}`}>
        ...
        {messages.length <= 1 && <div className="mt-8 p-6 bg-gray-800 border border-gray-700 rounded-lg">
            <h3 className="text-lg font-medium text-gray-300 mb-4">Common Questions:</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {commonQuestions.map((question, index) => <Button key={index} variant="outline" className="text-left h-auto p-4 text-white bg-gray-700 border-gray-600 hover:bg-gray-600 hover:border-gray-500 transition-all duration-200 justify-start" onClick={() => handleQuickQuestion(question)} disabled={isLoading}>
                  {question}
                </Button>)}
            </div>
          </div>}
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex space-x-3 items-end">
            <div className="flex-1 relative">
              <Input value={inputMessage} onChange={e => setInputMessage(e.target.value)} placeholder="Type your immigration question here..." className="bg-gray-800 border-gray-600 text-white placeholder-gray-400 pr-12 py-4 rounded-xl focus:border-blue-500 text-base" onKeyPress={e => e.key === 'Enter' && !isLoading && handleSendMessage()} disabled={isLoading} />
            </div>
            <Button onClick={() => handleSendMessage()} className="bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-xl" disabled={!inputMessage.trim() || isLoading}>
